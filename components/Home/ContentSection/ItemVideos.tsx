import { StyleSheet, Text, View,TouchableOpacity, ImageBackground, TouchableNativeFeedback, } from 'react-native'
...
const ItemVideos = ({ isSelected, data,onSelected }:ItemVideoProps) => {
    ...
    const ITEM_SELECTED:Array<string> = ["p-0 relative","w-[200]", "min-h-[200]" ,"rounded"," bg-gray-100", "overflow-hidden","text-xs"]
    isSelected ? ITEM_SELECTED.push("border-4 border-green-500 shadow") : ITEM_SELECTED
    

    return (<TouchableOpacity onPress={handleClick}   >
        <Card className={ITEM_SELECTED.join(' ')} >  
        <Animated.Image source={{uri:data?.posterUrl}} resizeMode="cover" style={styles.image}    />
